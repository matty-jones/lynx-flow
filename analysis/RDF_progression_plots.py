...
def plot_first_peaks(project):
    ...
    for plot_index, plot_type in enumerate(['Position (A)',
                                            'Magnitude (Arb. U.)']):
            ...
            for z_size in [10, 15, 20, 25]:
                ...
                for index, metal in enumerate(['Mo', 'Nb', 'Te', 'V']):
                    ...
                    for job in project.find_jobs(
                        ...
                        try:
                            first_peak = job.document['_'.join(
                                ['RDF_first_peak', metal])]
                            first_peak_data.append(first_peak[plot_index])
                            temperatures.append(job.sp()['temperature'])
                        except KeyError:
                            continue
                    plot_T, plot_peak = zip(*sorted(zip(temperatures,
                                                        first_peak_data)))
                    plt.plot(plot_T, plot_peak, color=colours[index],
                             label=metal)
                plt.xlabel('T (K)')
                plt.ylabel(plot_type)
                plt.title(''.join(['Dims = ', dimension, ', Z = ',
                                   str(z_size)]))
                plt.legend(prop={'size': 10})
                plot_file_name = ''.join(['first_peak_', plot_type[:3].lower(),
                                          '_', dimension, '_Z', str(z_size),
                                          '.pdf'])
                plot_save_loc = os.path.join('..', 'output_figures',
                                             plot_file_name)
