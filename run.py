...
if __name__ == "__main__":
    ...
    while True:
        ...
        if (first_run is False) and ('generated' not in label_progress.keys()):
            print("No simulations have been generated. Performing first run of project.py...")
            first_run = True
        elif ('generated' not in label_progress.keys()):
            print("`Generated' label not found. Probably nothing has finished yet. Sleeping...")
            T.sleep(300)
        elif label_progress['generated'] != 100.0:
            print("Current generation progress = {}%. Sleeping...".format(label_progress['generated']))
            T.sleep(300)
        elif (label_progress['generated'] == 100.0) and (second_run is False)\
                and (label_progress['simulated'] != 100.0):
            print("Generate complete!")
            print("All simulations have been generated, but not simulated. Performing second run of project.py...")
            second_run = True
        elif (label_progress['generated'] == 100.0) and (label_progress['simulated'] != 100.0):
            print("Current simulation progress = {}%. Sleeping...".format(label_progress['simulated']))
            T.sleep(300)
        elif (label_progress['generated'] == 100.0) and (label_progress['simulated'] == 100.0):
            print("All generations and simulations completed! Exiting...")
            break
        with open(log_file_name, 'a+') as log_file:
            subprocess.Popen(['python', 'project.py', 'run'], stdout=log_file)
